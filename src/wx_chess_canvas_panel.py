...
class ChessCanvasPanel(CanvasPanel):
    ...
    def __init__(self,
        ...
        :parent: Frame parent default: None passed to Frame
                :cgd: optional link to game display (ChessGameDisplay)
                    default: no cgd
        :app: applicatin ref
        ...
        :dark_sq: Color of dark squares
                default: "#a65" (brown)
        """
        self.is_window_setup = False    # Set True once window is fully set up
        ...
        if parent is None:
            ...
            cgd = parent         # Present == ChessGameDisplay
        self.cgd = cgd
    ...
    def OnSize(self, e):
        ...
        SlTrace.lg(f"{self.sq_size}")
        ps_w_init = 784
        ps_h_init = 749
        sq_w_init = 80
        sq_h_init = 80
        ps_w, ps_h = size
        sq_w = int(sq_w_init*(ps_w/ps_w_init))
        sq_h= int(sq_h_init*(ps_h/ps_h_init))
        sq_size = max(sq_w, sq_h)   # Use max
        SlTrace.lg(f"new: {sq_w=} {sq_h=} {sq_size=}")
        self.sq_size = sq_size
        self.set_changed()
        if self.is_window_setup:
            if self.cgd and self.cgd.display_board_count > 1:
